...
public class StreamExercise {
    ...
    boolean isPalindrome(String string) {
        String backWords = new StringBuilder(string).reverse().toString();
        return string.equalsIgnoreCase(backWords);
    }
    @Test
    public void task12() {
        String[] expected = {"Ada", "Ana", "Anna", "Ava", "Aya", "Bob", "Ebbe", "Efe", "Eje", "Elle", "Hannah", "Maram", "Natan", "Otto"};

        String[] result = people.stream().map(Person::getFirstName).collect(Collectors.toSet()).stream().filter(s -> isPalindrome(s)).sorted().toArray(String[]::new);
